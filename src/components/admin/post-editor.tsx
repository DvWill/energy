...
export function PostEditor({
  ...
  const bound = savePostAction.bind(null, post?.id ?? null),
    [state, action, pending] = useActionState<AdminFormState, FormData>(bound, {
      error: "",
    });
  const [title, setTitle] = useState(post?.title ?? ""),
    [slug, setSlug] = useState(post?.slug ?? ""),
    [slugTouched, setSlugTouched] = useState(Boolean(post));
  const [contentValue, setContentValue] = useState(post?.content ?? "<p></p>");
  ...
  const wrap = (before: string, after = before) => {
    ...
    el.setRangeText(
      `${before}${el.value.slice(start, end)}${after}`,
      start,
      end,
      "select",
    );
    setContentValue(el.value);
    el.focus();
  };
  return (
    <form action={action} className="post-editor">
      <div className="editor-main">
        <div className="admin-field">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (!slugTouched) setSlug(createSlug(e.target.value));
            }}
            required
            maxLength={180}
          />
          {state.fields?.title?.[0] && <small>{state.fields.title[0]}</small>}
        </div>
        <div className="admin-field">
        ...
        <fieldset>
          <legend>Fonte da notícia</legend>
          <div className="admin-field">
            ...
            />
            {state.fields?.sourceName?.[0] && (
              <small>{state.fields.sourceName[0]}</small>
            )}
          </div>
          <div className="admin-field">
            ...
            <input
              ...
              placeholder="https://…"
            />
            {state.fields?.sourceUrl?.[0] && (
              <small>{state.fields.sourceUrl[0]}</small>
            )}
          </div>
          <small>
            Preencha os dois campos para exibir a fonte e o link externo da
            notícia.
          </small>
        </fieldset>
        <div className="admin-field">
          ...
          />
          <small>
            HTML seguro: títulos, parágrafos, listas, links, imagens, citações e
            tabelas. O servidor sanitiza o conteúdo.
          </small>
        </div>
        <fieldset>
          <legend>SEO</legend>
          ...
          <div className="admin-field">
            <label htmlFor="canonicalUrl">Canonical personalizada</label>
            <input
              id="canonicalUrl"
              name="canonicalUrl"
              type="url"
          ...
          <div className="admin-field">
            <label htmlFor="socialImageUrl">Imagem social (URL)</label>
            <input
              id="socialImageUrl"
              name="socialImageUrl"
              type="url"
      ...
      <aside className="editor-sidebar">
        <fieldset>
          ...
          <div className="admin-field">
            ...
            <input
              ...
              name="publishedAt"
              type="datetime-local"
              defaultValue={localDate(post?.publishedAt)}
            />
          </div>
          <div className="admin-field">
            ...
            <input
              ...
              name="scheduledAt"
              type="datetime-local"
              defaultValue={localDate(post?.scheduledAt)}
            />
          </div>
          <label className="admin-check">
            <input
        ...
        <fieldset>
          ...
          <ImageUpload value={cover?.url ?? ""} onChange={setCover} />
          ...
          />
          <div className="admin-field">
            <label htmlFor="coverImageAlt">Texto alternativo</label>
            <input
              id="coverImageAlt"
              name="coverImageAlt"
          ...
          <div className="admin-field">
            <label htmlFor="coverImageCaption">Legenda</label>
            <input
        ...
        <button className="button" disabled={pending}>
          {pending ? "Salvando…" : "Salvar publicação"}
        </button>
      </aside>
    </form>
  );
