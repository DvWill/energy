...
export async function getFeaturedPost() {
  ...
  const rows = await db
    ...
    .leftJoin(categories, eq(posts.categoryId, categories.id))
    .leftJoin(authors, eq(posts.authorId, authors.id))
    .where(and(published(), eq(posts.isFeatured, true)))
    .orderBy(desc(posts.publishedAt))
    .limit(1);
  return rows[0] ?? null;
}
...
export async function getPublishedPost(slug: string) {
  ...
  const tagRows = await db
    ...
    .where(eq(postTags.postId, rows[0].post.id));
  ...
  const tagRows = await db
    ...
    .where(
      and(
        ...
        ...(ids.length ? [sql`${posts.id} not in ${ids}`] : []),
      ),
    )
    .orderBy(desc(posts.publishedAt))
    .limit(limit - categoryRows.length);
...
export async function adjacentPosts(publishedAt: Date, id: string) {
  ...
  const [previous, next] = await Promise.all([
    db
      ...
      .from(posts)
      .where(
        and(published(), ne(posts.id, id), lte(posts.publishedAt, publishedAt)),
      )
      .orderBy(desc(posts.publishedAt))
      .limit(1),
    db
...
export async function listAdminPosts(input: {
  ...
  if (
    ["DRAFT", "SCHEDULED", "PUBLISHED", "ARCHIVED"].includes(input.status ?? "")
  )
    filters.push(
      eq(
        posts.status,
...
export async function getAdminPost(id: string) {
  const rows = await getDb()
    .select({
      post: posts,
      scheduleHasStarted: sql<boolean>`
        ${posts.status} = 'SCHEDULED'
        and ${posts.scheduledAt} <= now()
      `,
    })
    .from(posts)
    ...
    .limit(1);
  if (!rows[0]) return null;
  if (!rows[0].scheduleHasStarted) return rows[0].post;
  return {
    ...rows[0].post,
    status: "PUBLISHED" as const,
    publishedAt: rows[0].post.scheduledAt,
    scheduledAt: null,
  };
}

export async function listAuthors() {
  return getDb().select().from(authors).orderBy(authors.displayName);
}
