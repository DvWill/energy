...
export async function deletePostAction(id: string) { await requireAdmin(); const db = getDb(); const item = await db.select({ slug: posts.slug }).from(posts).where(eq(posts.id, id)).limit(1); await db.delete(posts).where(eq(posts.id, id)); revalidatePath("/blog"); if (item[0]) revalidatePath(`/blog/${item[0].slug}`); revalidatePath("/sitemap.xml"); redirect("/admin/blog?deleted=1"); }
